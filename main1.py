...
class TimedPriorityQueue:
    ...
    def pop(self):
        ts,_,item = heapq.heappop(self.heap)
        return ts, item
...
class Peer:
    ...
    def sendtx(self,msg):
        for others in self.neighbor:
            if msg not in others.txpool:
                ...
                t = self.Delay(others,msg)
                others.txqueue[msg.txid] = t
                print(f'Transaction sent by {self.name} at {time1} to {others.name}')
                p = time1 + t
                tpq.push([others,2,msg],p)
    ...
    def generateTx(self,recv,amount):
        ...
        UTX.append(tx)
        print(f'Transaction generated by {self.name} at {time1} to {recv.name} for {amount} coins')
        self.sendtx(tx)
        return
...
class Network:
    ...
    def createNetwork(self):
        ...
        for i in range(self.n):
            ...
            tpq.push([self.all_peers[i],1,k,amount],random.randint(1,100))
        return G
...
if __name__ == "__main__":
    ...
    network = Network(arg1,arg2,arg3)
    ...
    while tpq.heap:
        ...
        if item[1] == 1:
            time1 = ts
            item[0].generateTx(item[2],item[3])

        if item[1] == 2:
            time1 = ts
            item[0].sendtx(item[2])